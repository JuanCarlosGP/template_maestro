...
// Preflight / environment doctor. Run via `make doctor` (or `npm run doctor`).
// Reports whether every prerequisite for running the suite is in place and exits
// non-zero if a HARD requirement is missing, so it can gate `make setup`.
//
...
try {
  ...
  fail('npm dependencies missing — run `make install`')
}
...
try {
  ...
  warn('Azure DevOps MCP server not installed locally — `make install` will fetch it (npx falls back to network)')
}

const maestro = run('maestro --version')
maestro ? ok(`Maestro CLI ${maestro}`) : fail('Maestro CLI not found — run `make install-maestro`')
...
booted.length ? booted.forEach(l => ok(`iOS booted: ${l.trim()}`)) : warn('no iOS simulator booted (`make simulator-start`)')
const droid = (run('adb devices') || '').split('\n').slice(1).filter(l => /\bdevice\b/.test(l))
droid.length ? ok(`Android: ${droid.length} device(s) connected`) : warn('no Android device/emulator (`make emulator-start`)')

// --- Config (soft) ------------------------------------------------------------
console.log('\nConfig:')
fs.existsSync(path.join(ROOT, '.env')) ? ok('.env present') : warn('.env missing — run `make env`')
process.env.AZURE_DEVOPS_PAT
  ? ok('AZURE_DEVOPS_PAT available (used by the runner for publishing results)')
  : warn('AZURE_DEVOPS_PAT not in this env — `make` reads it from .env for publishing; the Azure MCP is independent (interactive browser login)')
